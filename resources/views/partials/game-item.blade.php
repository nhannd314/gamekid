<div class="col-6 col-md-3">
    <a href="{{ route('game.show', $game->slug) }}" class="h-100 game-card bg-white rounded-3 overflow-hidden d-block shadow hover-bounce">
        ...
        <div class="py-2 py-md-3">
            ...
            <div class="text-center small fw-normal mb-1 d-flex justify-content-center gap-1 flex-wrap">
                @if ($game->min_age)
                    <span class="text-bg-light border rounded label-sm">{{ $game->min_age }}+ tuổi</span>
                @endif
                <span class="text-white rounded mb-1 label-sm {{ $game->difficultyBadgeClass() }}" style="min-width: 50px">{{ $game->difficultyLabel() }}</span>
            </div>
